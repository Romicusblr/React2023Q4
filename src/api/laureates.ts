...
export const laureatesApi = createApi({
  ...
  endpoints: (builder) => ({
    searchLaureates: builder.query<SearchLaureatesResponce, { name: string, page: number, limit: number }>({
      ...
      transformResponse: (response: SearchLaureatesDTO): SearchLaureatesResponce => {
        return {
...
export const { useSearchLaureatesQuery, useGetLaureateDetailsQuery } = laureatesApi;
